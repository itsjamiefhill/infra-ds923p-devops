...
- **Host Path**: `/volume1/nomad/volumes/vault_data` (default)
...
nomad alloc exec -task vault <vault-alloc-id> vault operator init > /volume1/nomad/config/vault-init.txt
...
sudo tar -czf /volume2/backups/services/vault_backup.tar.gz -C /volume1/nomad/volumes vault_data
...
2. Include `/volume1/nomad/volumes/vault_data` in the backup
